...
//STEP 0	
	if(factorCate == 'B'){
		...
		var queryStr = 'SELECT * FROM "SMART_OPERATION"."SMOPS_MASTER" \
			WHERE CUSTOMER_ID = ? \
			AND SYSID = ? \
			AND SYSCLT = ? \
			AND FACTOR_CATEGORY = ? \
			AND FACTOR_NAME IN \
				(SELECT TOP 5 TABLE_NAME FROM "SMART_OPERATION"."CMTBL" \
					WHERE CUSTOMER_ID = ? \
					AND SYSTEM_ID = ? \
					AND SYSTEM_CLT = ? \
					AND DATE_Y = (SELECT MAX(DATE_Y) FROM "SMART_OPERATION"."CMTBL" WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ?) \
					AND DATE_M = (SELECT MAX(DATE_M) FROM "SMART_OPERATION"."CMTBL" WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ? AND DATE_Y = (SELECT MAX(DATE_Y) FROM "SMART_OPERATION"."CMTBL" WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ?)) \
				ORDER BY TABLE_ENTRIS_CUM DESC) \
			ORDER BY TREND DESC';
		...
		var queryStr = 'SELECT * FROM "SMART_OPERATION"."SMOPS_MASTER" \
			...
			AND TREND < 9999.99 \
			AND FACTOR_NAME IN \
				( SELECT TOP 5 REPORT_NAME FROM "SMART_OPERATION"."CMWLP" WHERE \
					CUSTOMER_ID = ? \
					AND SYSTEM_ID = ? \
					AND SYSTEM_CLT = ? \
					AND DATE_Y = (SELECT MAX(DATE_Y) FROM "SMART_OPERATION"."CMWLP"  WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ?) \
					AND DATE_M = (SELECT MAX(DATE_M) FROM "SMART_OPERATION"."CMWLP"  WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ? AND DATE_Y = (SELECT MAX(DATE_Y) FROM "SMART_OPERATION"."CMWLP" WHERE CUSTOMER_ID = ? AND SYSTEM_ID = ? AND SYSTEM_CLT = ?)) \
					GROUP BY "REPORT_NAME","TASK_TYPE","CUSTOMER_ID","SYSTEM_ID","SYSTEM_CLT" \
				ORDER BY SUM( "RESP_TOTAL" ) DESC) \
			ORDER BY TREND DESC';
