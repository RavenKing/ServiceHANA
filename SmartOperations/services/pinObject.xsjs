...
function handleGet() {
	...
	var pin = $.request.parameters.get("pin");
	var setPin = '';
	...
	var checkStr = 'UPDATE "SMART_OPERATION"."PREDICT_FACTOR_MASTER" SET PIN = ? WHERE FACTOR_GUID = ?';
	var kst = connCheck.prepareStatement(checkStr);
	kst.setString(1,setPin);
	kst.setInteger(2,parseInt(factorId));
	kst.execute();
	kst.close();
	connCheck.commit();
	connCheck.close();
